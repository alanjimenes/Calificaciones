...
function renderTable() {
    ...
    if(tableBody) {
        ...
        currentStudents.forEach((student, index) => {
            ...
            let rowHTML = `
                <td class="p-0 text-center text-text-secondary/50 font-mono text-xs">${index + 1}</td>
...
if (formStudent) {
    formStudent.addEventListener('submit', async (e) => {
        ...
        if (!name || !id) return;
        if (currentStudents.some(s => s.id === id)) { alert("ID Duplicado"); return; }
        currentStudents.push({ id: id, nombre: name, notas: {}, asistencia: {} });
        await updateDoc(doc(db, "cursos_globales", COURSE_ID), { estudiantes: currentStudents });
