...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Configuración para instancia temporal
    ...
    if (formRegister) {
        formRegister.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                overlay.innerHTML = `
                    <div class="bg-surface-dark border border-primary/30 p-8 rounded-2xl shadow-2xl max-w-md w-full text-center relative overflow-hidden">
                        ...
                        <p class="text-text-secondary text-sm mb-6">
                            La cuenta <strong>${email}</strong> ha sido creada correctamente.
                        </p>
                ...
                // 2. Esperar 3.5 segundos, cerrar sesión y redirigir
                setTimeout(async () => {
                    try {
                        await signOut(auth); // Cerramos la sesión del Admin
